...
def get_problem(cfg):
    if cfg["problem"] == "heat_1d":
        ...
        raise ValueError("Problema 'magnetostatic_mesh' foi desabilitado temporariamente por falta de suporte confiável no pipeline atual.")
    elif cfg["problem"] == "magnetodynamic_mesh":
        raise ValueError("Problema 'magnetodynamic_mesh' foi desabilitado temporariamente por falta de suporte confiável no pipeline atual.")
    else:
