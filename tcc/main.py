...
def main():
    ...
    if CONFIG.get("use_mesh") and "fem_data" in problem:
        try:
            fem_data = problem["fem_data"]
            nodes        = fem_data["nodes"]
            nodeTags     = fem_data["nodeTags"]
            triElements  = fem_data["triElements"]
            elements     = fem_data["elements"]
            boundaryConditions = fem_data["boundaryConditions"]
            ...
            if fem_solver:
                ...
                fem_solver.solve()
                fem_solver.calculate_electric_field()
                ...
                print(f"✓ Malha FEM: Bounds [{x_min:.2f}, {x_max:.2f}] x [{y_min:.2f}, {y_max:.2f}]", flush=True)

        except Exception as e:
            print(f"Erro ao resolver FEM: {e}", flush=True)
