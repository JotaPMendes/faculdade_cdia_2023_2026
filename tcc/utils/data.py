...
def generate_data_for_ml(problem, cfg, model_fem=None):
    ...
    if use_mesh:
        ...
        if len(Xcand) == 0:
            print("ℹ️ Train Box cobre todo o domínio da malha. Usando pontos aleatórios para teste (Interpolação).")
            # Gerar pontos de teste aleatórios dentro do domínio (interpolação)
            Xte = np.random.rand(N // 2, 2)
            Xte[:,0] = bx0 + Xte[:,0] * (bx1 - bx0)
            Xte[:,1] = by0 + Xte[:,1] * (by1 - by0)
            yte = u_true(Xte)
            mask_valid = ~np.isnan(yte)
            Xte = Xte[mask_valid]
            yte = yte[mask_valid]
