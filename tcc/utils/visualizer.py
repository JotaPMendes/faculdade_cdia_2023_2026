...
def plot_mesh(run_dir, config, pinn_instance, problem):
    ...
    fig.update_layout(
        ...
        plot_bgcolor="#1e1e1e",
        height=920,
        margin=dict(t=125, b=55, l=55, r=160),
        legend=dict(
            x=1.02,
            y=0.98,
            orientation="v",
        ...
        shapes=[
            dict(
                ...
                x0=0.02, y0=0.08, x1=0.53, y1=0.93,
                line=dict(color="#3b4252", width=1.5),
                fillcolor="rgba(42, 46, 58, 0.28)",
                layer="below"
            ),
            dict(
                type="rect",
                xref="paper", yref="paper",
                x0=0.59, y0=0.55, x1=0.97, y1=0.93,
                line=dict(color="#3b4252", width=1.5),
                fillcolor="rgba(42, 46, 58, 0.22)",
                layer="below"
            ),
            dict(
                type="rect",
                xref="paper", yref="paper",
                x0=0.59, y0=0.08, x1=0.97, y1=0.45,
                line=dict(color="#3b4252", width=1.5),
                fillcolor="rgba(42, 46, 58, 0.22)",
                layer="below"
            ),
            dict(
                type="line",
                xref="paper", yref="paper",
                x0=0.56, y0=0.08, x1=0.56, y1=0.93,
                line=dict(color="rgba(120, 130, 150, 0.35)", width=1)
            ),
            dict(
                type="line",
                xref="paper", yref="paper",
                x0=0.59, y0=0.50, x1=0.97, y1=0.50,
                line=dict(color="rgba(120, 130, 150, 0.35)", width=1)
            ),
        ],

        scene=dict(
            domain=dict(x=[0.03, 0.52], y=[0.08, 0.93]),
            bgcolor="#23262f",
    ...
    fig.update_annotations(font=dict(size=14), yshift=8)

    fig.update_xaxes(
        showgrid=True,
        gridcolor="rgba(100, 150, 255, 0.10)",
        zeroline=False,
        showline=True,
        linecolor="#546072",
        mirror=False,
    ...
    fig.update_xaxes(
        showgrid=True,
        gridcolor="rgba(255, 120, 120, 0.08)",
        zeroline=False,
        showline=True,
        linecolor="#546072",
        mirror=False,
